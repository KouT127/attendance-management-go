...
func testUserToManyAttendances(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, attendanceDBTypes, false, attendanceColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, attendanceDBTypes, false, attendanceColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Attendances().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadAttendances(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Attendances); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Attendances = nil
	if err = a.L.LoadAttendances(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Attendances); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpAttendances(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e Attendance

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*Attendance{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, attendanceDBTypes, false, strmangle.SetComplement(attendancePrimaryKeyColumns, attendanceColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Attendance{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddAttendances(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Attendances[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Attendances[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Attendances().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpAttendances(t *testing.T) {
	var err error
	...
	err = a.SetAttendances(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetAttendances(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.Attendances[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.Attendances[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpAttendances(t *testing.T) {
	var err error
	...
	err = a.AddAttendances(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveAttendances(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.Attendances) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.Attendances[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.Attendances[0] != &e {
...
var (
	userDBTypes = map[string]string{`ID`: `varchar`, `Email`: `varchar`, `Name`: `varchar`, `ImageURL`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_           = bytes.MinRead
)
